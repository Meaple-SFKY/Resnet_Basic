...
class Net(object):
    def __init__(self, device='cpu') -> None:
        self.device = device
        self.conv1 = Conv2D(in_channels=1, out_channels=8, kernel_size=5, input_shape=28, stride=1, padding=0, batchsize=batch_size, bias=True, const_ker=False, device=self.device)
        self.pool1 = MaxPooling((batch_size, 8, 24, 24), 2, 2, device=self.device)
        self.relu1 = Relu((batch_size, 8, 12, 12), device=self.device)

        self.conv2 = Conv2D(in_channels=8, out_channels=32, kernel_size=3, input_shape=12, stride=1, padding=0, batchsize=batch_size, bias=True, const_ker=False, device=self.device)
        self.pool2 = MaxPooling((batch_size, 32, 10, 10), 2, 2, device=self.device)
        self.relu2 = Relu((batch_size, 32, 5, 5), device=self.device)

        self.conv3 = Conv2D(in_channels=32, out_channels=32, kernel_size=2, input_shape=5, stride=1, padding=0, batchsize=batch_size, bias=True, const_ker=False, device=self.device)
        self.pool3 = MaxPooling((batch_size, 32, 4, 4), 2, 2, device=self.device)
        self.relu3 = Relu((batch_size, 32, 2, 2), device=self.device)

        self.linear1 = Linear((batch_size, 32, 2, 2), 128, 32, True, device=self.device)
        self.linear2 = Linear((batch_size, 32), 32, 10, True, device=self.device)
...
if __name__ == '__main__':
    ...
    softmax = SoftMax((batch_size, 10), device=device)
    ...
    for epoch in range(20):
        ...
        for i in tqdm(range(int(train_cnt / batch_size))):
            image = images[i * batch_size:(i + 1) * batch_size].reshape([batch_size, 1, 28, 28]).to(device)
            label = labels[i * batch_size:(i + 1) * batch_size].to(device)
